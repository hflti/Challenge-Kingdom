        ...
        <div className="parent-rewards-grid">
          {saved.childContent.storeItems.map((item, index) => (
            <article className="parent-reward-card" key={item.id}>
              ...
              <label className="admin-field"><span>السعر بالنقاط</span><input className="admin-input" type="number" min={1} step={1} value={item.cost} onChange={(event) => onUpdateReward(index, { cost: Math.max(1, Math.round(Number(event.target.value) || 1)) })} /></label>
