...
export function normalizeChildRewards(value: unknown): ChildRewardsState {
  ...
  return {
    ...
    badgeCount: typeof item.badgeCount === "number" ? Math.min(3, Math.max(0, Math.floor(item.badgeCount))) : 0,
...
export function ChildExtras({ content, points, completed, rewards, childName, childAvatar, onSpend, onOpenBox, onOpenBadge, onSelectGift, onAwardPoints, onRequestUnlock, onFinishStory, onStartStory }: {
  ...
  const revealGifts = () => {
    if (rewards.badgeCount < 3 || content.majorBoxRewards.length !== 3 || content.displayBoxRewards.length !== 2) return;
    clearGiftShuffle();
    setGiftOrder(randomGiftOrder());
  ...
  const chooseRandomGift = (boxIndex: number) => {
    if (giftStage !== "ready" || content.majorBoxRewards.length !== 3 || content.displayBoxRewards.length !== 2) return;
